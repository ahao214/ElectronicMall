/*
Desc:仓储相关接口
EF Core只是对大多数关系数据库和少数非关系数据库的抽象，并没有彻底摆脱对细节的依赖。
该接口文件是希望项目不依赖任何具体细节，因此准备了这套和底层存储系统毫无关系的抽象仓储接口
Author:Joker
CreatedDate:24-02-01
*/

using ElectronicMall.Domain.Abstractions.Entities;

namespace ElectronicMall.Domain.Abstractions.Repositories
{
    ...
    public interface IBulkOperableVariableRepository<TResult, TVariableRepository, TEntity> where TEntity : IEntity
    ...
    /// <typeparam name="TVariableRepository">基础仓库类型</typeparam>
    /// <typeparam name="TEntity">实体类型</typeparam>
    public interface IBulkOperableVariableRepository<TVariableRepository, TEntity> where TEntity : IEntity
        where TVariableRepository : IVariableRepository<TEntity>
    {
        /// <summary>
        /// 保存变更
